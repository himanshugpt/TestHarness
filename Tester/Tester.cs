...
namespace TestHarness
{
  public class Tester
  {
   ...
   void LoadAndRun(string destDir)
    {
      ...
      TextWriter tw = new StreamWriter(destDir + "\\results.xml");
      // write a line of text to the file
      tw.WriteLine(results_);
      // close the stream
      tw.Close();
      Console.WriteLine("\nTesting finished. Now unloading the Add Domain..");
    }

    //
    //----< Run tests in configDir >---------------------------------

   public void runTests(Object destDir)
    {
      try
      {
        CreateAppDomain();
        LoadAndRun(destDir.ToString());
        UnloadTestDomain();
      }
      catch(Exception ex)
      {
    ...
    public Thread SelectConfigAndRun(string configDir)
    {
      pathToTestLibs_ = configDir;
      Thread t = new Thread((this.runTests));
      t.Start(configDir);
      return t;
    }
  }
